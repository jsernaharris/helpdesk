...
<div class="max-w-3xl">
    <form method="POST" action="{{ route('staff.kb.update', $article) }}" class="bg-white shadow rounded-lg p-6 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div><label class="block text-sm font-medium text-gray-700">Visibility</label><select name="visibility" class="mt-1 block w-full rounded-md border-gray-300 text-sm px-3 py-2 border"><option value="public" @selected($article->visibility === 'public')>Public</option><option value="internal" @selected($article->visibility === 'internal')>Internal</option><option value="customer_specific" @selected($article->visibility === 'customer_specific')>Customer Specific</option></select></div>
        ...
        <div><label class="block text-sm font-medium text-gray-700">Content</label><textarea name="content" rows="12" required class="mt-1 block w-full rounded-md border-gray-300 text-sm px-3 py-2 border font-mono">{{ $article->content }}</textarea></div>

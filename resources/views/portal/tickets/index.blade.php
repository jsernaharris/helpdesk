...
<div class="flex justify-between items-center mb-6">
    <form method="GET" class="flex gap-3">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search tickets..." class="rounded-md border-gray-300 text-sm px-3 py-2 border">
        <select name="status" class="rounded-md border-gray-300 text-sm px-3 py-2 border" onchange="this.form.submit()">
            <option value="">All Statuses</option>
            @foreach(['new','open','pending','on_hold','resolved','closed'] as $s)
    ...
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
            ...
            <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location='{{ route('portal.tickets.show', $ticket) }}'">
                ...
                <td class="px-4 py-3 text-sm text-gray-500">{{ $ticket->assignedTo?->name ?? 'Pending' }}</td>
                ...
                <td class="px-4 py-3 text-sm text-gray-500">{{ $ticket->updated_at->diffForHumans() }}</td>
            </tr>
            @empty
            <tr><td colspan="6" class="px-4 py-8 text-center text-sm text-gray-500">No tickets found.</td></tr>
            @endforelse
        </tbody>
    </table>
